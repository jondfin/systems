...
int main(int argc, char **argv){
	...
	*count = 0;
        ...
        if(argc == 3){
                dirHandler(directory, "", argv[2], ppid, count);
        }
        //if there are 5 arguments, can either be -d or -o as the 3rd argument
        else if(argc == 5){
                if(strcmp(argv[3], "-d") == 0){
                        dirHandler(argv[4], "", argv[2], ppid, count);
                } else {
...
void dirHandler(char *dirname, char *outputdir, char *colname, int ppid, int *count){
	...
	if(!(dir = opendir(dirname))){
		fprintf(stderr, "\nCannot open starting directory, %s\n", strerror(errno));
		printf("Total number of processes: 0\n");
		exit(0);
	}
	if (strcmp(outputdir, "") != 0){
		DIR *dir2;
		if(!(dir2 = opendir(outputdir))){
			fprintf(stderr, "\nThe output directory does not exist, %s\n", strerror(errno));
			printf("Total number of processes: 0\n");
			exit(0);
		} else {
			closedir(dir2);
...
void sort(char *csvFileName, char *outputdir, char *colName)
{
	...
	char *csvName = malloc(sizeof(char)*strlen(tempcsvName));
        ...
        if(numberOfRecords < 2){
			fprintf(stderr, "Error: Not enough records in %s to sort on", csvName);
			exit(EXIT_FAILURE);
		}
