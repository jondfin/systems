...
//		printf("No files read. Exiting\n");
	...
	//TODO
	//Implement mergesort -> how to sort on random headers
...
void writeToFile(Node **arr, char *colname, char *outputdir){
	...
	if(strcmp(outputdir, "") == 0){
		char *path = (char*)malloc((strlen(colname)+strlen(outputdir)+22)*sizeof(char));
		path = strcpy(path, outputdir);
		path = strcat(path, "/");
		path = strcat(path, filename);
		path = strcat(path, "\0");
		filename = realloc(filename, strlen(path)*sizeof(char));
...
void getToken(Node **arr, char *line, int index){
	...
	for(i = 0; i < strlen(line); i++){
		...
		if(line[i] == '\"'){
			...
			token[k] = '\"';
			arr[index] = insert(arr[index], -1, token);
			i++;
			k = 0;
			memset(token, 0, 256);
		}
...
Node *insert(Node *front, pthread_t tid, char *line){
