...
Node *copy(Node *dest, Node *src){
	...
	while(p2 != NULL){
		p1->line = (char*)malloc(strlen(p2->line)*sizeof(char)+1);
		strcpy(p1->line, p2->line);
		p1 = p1->next;
		p2 = p2->next;
	}
	return dest;
...
void merge(Node **arr, int left, int mid, int right, int colNum, int numCols){
	...
	while(i < sizeA1 && j < sizeA2){
		...
		if(isdigit(trim(p1->line)[0]) && isdigit(trim(p2->line)[0])){
			/*Convert to double to hold data*/
			double num1 = atof(p1->line);
			double num2 = atof(p2->line);
			if(num1 <= num2){
			...
			switch(strcmp(trim(p1->line), trim(p2->line))){
				case -1:/*Less than*/
					arr[k] = copy(arr[k], A1[i]);
					i++;
					break;
