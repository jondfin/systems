...
int main(int argc, char **argv){
	...
	/*Error checking to make sure all the inputs are in correct order*/
	if(argc < 3){
 	...
 	if(argc == 3){
		if(strcmp(argv[1], "-c") != 0){
			fprintf(stderr, "%s\n", "Error: Must specify -c and a column to sort on in order to run the program.");
			exit(EXIT_FAILURE);
		}
	}
    	...
    	if(argc == 4){
		fprintf(stderr, "%s\n", "Error: Must either specify a starting directory, output directory, or column name depending on the third parameter.");
	        exit(EXIT_FAILURE);
    	}
    	if(argc == 5){
		if(strcmp(argv[1], "-c") != 0){
		...
		if(strcmp(argv[1], argv[3]) == 0){
			fprintf(stderr, "%s\n", "Error: Cannot have repeating parameters.");
			exit(EXIT_FAILURE);
		}
	}
 	if(argc > 5 && strcmp(argv[5], "-o") != 0){
			...
			if(strcmp(argv[5], "-c") != 0){
				fprintf(stderr, "%s\n", "Error: Invalid fifth parameter.");
				exit(EXIT_FAILURE);
			}
		}
  	}
  	if(argc == 6){
		fprintf(stderr, "%s\n", "Error: Must specify an output directory, starting directory, or column name depending on the last parameter inputted.");
        	exit(EXIT_FAILURE);
    	}
   	if (argc == 7){
		if (strcmp(argv[1], "-c") != 0){
		...
		if ((strcmp(argv[1],argv[3]) == 0 || strcmp(argv[1],argv[5]) == 0) || (strcmp(argv[3],argv[5]) == 0)){
			fprintf(stderr, "%s\n", "Error: Cannot have duplicate parameters.");
            		exit(EXIT_FAILURE);
       		}
    	}
    	if(argc > 7){
	...
	//if only 3 inputs, then -c <columnNameToSortBy> is the only input, start from cwd and leave output directory blank for now
	if(argc == 3){
		dirname = getcwd(cwd, sizeof(cwd));
		outputdir = getcwd(cwd, sizeof(cwd));
		colname = argv[2];
	}
	//if there are 5 arguments, check to see which one is the column
	else if(argc == 5){
		if(strcmp(argv[1], "-c") == 0){
	...
	//if all parameters entered, then they may be in any order
	else if(argc == 7){
		if(strcmp(argv[1], "-c") == 0){
			if(strcmp(argv[3],"-d") == 0){
				dirname = argv[4];
...
void *fileHandler(void *Args){
	...
	if(strstr(a->name, ".csv")){
		...
		if(strstr(headers, validMovieHeadings) == NULL){
